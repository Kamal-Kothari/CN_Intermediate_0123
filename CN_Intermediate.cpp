...
for(int val:v)
{
	sum=max(sum+val,0);//continue previous sum if >0 or start new subarray with sum=0
...
vector<int> sumInRanges(vector<int> &arr, int n, vector<vector<long long>> &queries, int q) {
   ...
   // It stores answer for each query.    vector<int> ans;
...
//operation take a char and put it to end
//O(n) O(n)
#include <bits/stdc++.h>
...
//56 smallest subarray with k distinct elements
//O(n) O(k)
#include <bits/stdc++.h>
...
{   
    ...
    while(x<a && y<b && z<c)
    {
        ...
        if(p<=q && p<=r) x++;
        else if(q<=p and q<=r) y++;//ans instead of &&
        else z++;
    }
    return ans;
}
...
//62 check duplicate element present in window of k size
//sc O(n)
#include <bits/stdc++.h> 
bool checkDuplicate(vector<int> &arr, int n, int k) {
    // Write your code here.
    unordered_map<int,int> um;//1 based idx
    //val vs idx
    for(int i=0;i<n;i++)
    {
        if(um[arr[i]]>0)
        {
            if(i+1-um[arr[i]]<=k) return true;            
        }
        um[arr[i]]=i+1;
    }
    return false;
